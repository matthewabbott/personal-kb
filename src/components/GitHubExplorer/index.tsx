...
export function GitHubExplorer() {
            ...
            try {
                ...
                const reposWithPreviews = await Promise.all(
                    sortedRepos.map(async (repo) => {
                        const cachedPreview = getCachedData<string>(`readme-preview-${repo.name}`);
                        if (cachedPreview !== null) {
                            return { ...repo, readme_preview: cachedPreview };
                        }
                        ...
                        try {
                            ...
                            if (readmeResponse.ok) {
                                ...
                                const preview = truncateMarkdown(readme);
                                setCachedData(`readme-preview-${repo.name}`, preview);
                                console.log(`Cached README preview for ${repo.name}`);
                                return { ...repo, readme_preview: preview };
                            }

                            if (readmeResponse.status === 404) {
                                console.log(`No README found for ${repo.name}`);
                                // Cache the fact that there's no README
                                setCachedData(`readme-preview-${repo.name}`, null);
    ...
    const fetchReadme = async (repo: Repository) => {
        ...
        if (readmeCache[repo.name]) {
            console.log(`Cache hit (memory): readme for ${repo.name}`);
            setExpandedRepo({
                id: repo.id,
        ...
        if (cachedReadme !== null) {
            console.log(`Using cached README from localStorage for ${repo.name}`);
        ...
        console.log(`Fetching README from GitHub API for ${repo.name}`);
